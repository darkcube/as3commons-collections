...
package org.as3commons.collections.fx.iterators {
	...
	/**
	 * Implementation of the <code>Map</code> iterator that also implements <code>IViewCursor</code>.
	 * 
	 * @author Timothy Lusk 11.23.2011
	 */
	public class MapIteratorFx extends MapIterator implements IViewCursor {
		...
		/**
		 * MapIteratorFx constructor.
		 * 
		 * @param map The map to enumerate.
		 */
		public function MapIteratorFx(sortedMap : MapFx) {
			sortedMap.addEventListener(CollectionEvent.COLLECTION_CHANGE, collectionEventHandler, false, 0, true);
		...
		{
			return new MapIteratorBookmark(CursorBookmark.CURRENT.value, index);
		}
		...
		public function seek(bookmark:CursorBookmark, offset:int = 0, prefetch:int = 0):void
		{
			...
			{
				newIndex = MapIteratorBookmark(bookmark).currentIndex;
			}
...
/**
 *  @private
 *  Encapsulates the positional aspects of a cursor within an MapIteratorBookmark.
 *  Only the MapIteratorBookmark should construct this.
 */
internal class MapIteratorBookmark extends CursorBookmark
{
	 ...
	 *  @private
	 */
	public function MapIteratorBookmark(value:Object,
										currentIndex:int)
	{
		super(value);
